...
export default function NavBar({ projectsRef, experienceRef } : { projectsRef: React.RefObject<HTMLElement>, experienceRef: React.RefObject<HTMLElement> }) {
    ...
    return (
        <div className={`w-full flex flex-col justify-center bg-black text-white fixed px-6 transition-all duration-300 ${menu ? "bg-opacity-100" : "bg-opacity-80"}  ${scroll ? "h-16" : "h-24"}`}>
